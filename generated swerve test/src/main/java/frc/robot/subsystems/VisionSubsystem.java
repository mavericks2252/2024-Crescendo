...
public class VisionSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    /**
     * addVisionRobotPose();
     * Pose2d currentRobotPos = drivetrain.getState().Pose; //sets currentRobotPos
  ...
  public Pose2d getSpeakerTargetRotation2d() {
    ...
    // double xRobotPosMeters =
    // LimelightHelpers.getBotPose2d_wpiBlue(limelight).getX(); //get xpose of the
    // robot
    // double yRobotPosMeters =
    // LimelightHelpers.getBotPose2d_wpiBlue(limelight).getY(); //get the ypose of
    // the robot
  ...
  public double getTotalLatency() { // returns total latency in seconds
    double tl = LimelightHelpers.getLatency_Pipeline(limelight); // gets the latency from the pipeline
    double cl = LimelightHelpers.getLatency_Capture(limelight); // gets the latency from the capture rate
    SmartDashboard.putNumber("total Latencey", tl + cl); // adds the latencies and puts them onto the dashboard
    return (tl + cl) / 1000; // returns the total latency
  }
  ...
  public void addVisionRobotPose() {
    ...
    if (tv && ta > .75) {
      drivetrain.addVisionMeasurement(getRobotPoseVision(), Timer.getFPGATimestamp() - getTotalLatency()); // corrects
                                                                                                           // the
                                                                                                           // odometry
                                                                                                           // pose and
                                                                                                           // takes in
  ...
  public double getSpeakerDistance() {
    ...
    double botX = drivetrain.getState().Pose.getX(); // gets the x pos of the robot
    double botY = drivetrain.getState().Pose.getY(); // gets the y pos of the robot
    double oppositetSide = botY - FieldConstants.kBlueSpeakerYPosMeters; // uses the robot pos to determine the distance
                                                                         // to the speaker in the y direction

    return Math.hypot(oppositetSide, botX); // does the math to find the distance to the speaker
  ...
  public double getTargetAngle() {
    ...
    return Math.toDegrees(Math.atan2(speakerToPivot, getSpeakerDistance())); // gets the tangent angle of the speaker
                                                                             // from the pivot of the shooter
  }

  public double getAmpDistance() {
    double botX = drivetrain.getState().Pose.getX(); // gets the x pos of the robot
    double botY = drivetrain.getState().Pose.getY(); // gets the y pos of the robot
    double oppositetSide = botY - FieldConstants.kBlueAmpYPosMeters; // finds the y distance from the amp to the bot

    return Math.hypot(oppositetSide, botX); // calculates the distance from the current bot pose to the amp
  }
   ...
   * 
   * return
   * LimelightHelpers.getBotPose2d_wpiBlue(limelight).getRotation().getDegrees();
   * 
   * }
   * 
   * public double getSpeakerDistance() {
